...
from services.docx_service import render_docx
...
def gerar_lote(payload: BatchGenerateReportPayload, current_user=Depends(get_current_user)):
    ...
    with zipfile.ZipFile(zip_buffer, "w", compression=zipfile.ZIP_DEFLATED) as zip_file:
        for patient_data in payload.patients:
            try:
                ...
                document_bytes = render_docx(resolve_storage_path(template["file_path"]), patient_data)
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'attachment; filename="laudos.zip"'},
    )
